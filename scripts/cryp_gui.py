    ...
    <body>
        ...
        微信扫码使用此插件</div>
                ...
                with gr.Accordion("起手式", open=True):
                    ...
                    with FormRow():
                        with FormColumn(min_width=160):
                            btnStyleXL = gr.Button("XL起手式")
                        with FormColumn(elem_id="Randomize Style"):
                            btnStyle15 = gr.Button("1.5起手式")
                    with FormRow():
                        selected = self.styleNames[0]
                        style = gr.Dropdown(self.styleNames, value=selected, multiselect=True,visible=True, label="选择XL起手式")
                        self.styleNames15 = changeStyles('sd1.5_styles.json')
                        selected = self.styleNames15[0]
                        style15 = gr.Dropdown(self.styleNames15, value=selected, multiselect=True,visible=False, label="选择1.5起手式")
            ...
            with gr.TabItem("登录", id="login"):
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        if install_result==0:
                            install_btn = gr.Button("安装解密模块")
                            gr.HTML('<button class="lg secondary gradio-button svelte-cmf5ev" style="color:#808080;width:100%" id="component-341466"> 卸载解密模块</button>')
                        else:
